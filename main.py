...
def show_post(post_id):
    ...
    if form.validate_on_submit() and current_user.is_authenticated:
        ...
        new_comment=CommentTable(Text=text,comment_author=current_user,post=requested_post)
        db.session.add(new_comment)
        db.session.commit()
    else:
        flash("You need to login or sign up to comment on the posts")

    comments=db.session.execute(db.select(CommentTable).where(CommentTable.post_id==post_id)).scalars().all()
...
def delete_comment(comment_id):
    comment_to_delete = db.get_or_404(CommentTable, comment_id)
    db.session.delete(comment_to_delete)
    db.session.commit()
    return redirect(url_for('show_post', post_id=comment_to_delete.id))  # Adjust if your Comment model has a relation
